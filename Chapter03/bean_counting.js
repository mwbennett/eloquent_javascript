...
similar to how you get its length with "s".length. The returned value will be a string containing only one 
character (for example, "b"). The first character has position zero, which causes the last one to be found 
at position string.length - 1. In other words, a two-character string has length 2, and its characters have 
positions 0 and 1.
...
var countBs = function(str) {
  ...
  for (var position = 0; position < str.length; position++) {
    if (str.charAt(position) == "B") countBs += 1;
  }
  return countBs;
}
